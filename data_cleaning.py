...
def override_missing_values(raw_data, years_wd = 2): #inputs values with the average
...
def preprocess_country_data(country, raw_filenamePT, raw_filenameSW):
    if country == "PT":
        data = pd.read_csv(raw_filenamePT, header=None, sep='\s+', names=["Year", "Age", "mxf", "mxm",'mxTotal'])
        data = preprocessing_dataPT(data)
        if not exists('Dataset/dataPT.csv'):
            data.to_csv('Dataset/dataPT.csv', index=False)
            
    elif country == "SW":
        data = pd.DataFrame(pd.read_excel(raw_filenameSW))
