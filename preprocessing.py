...
def transformer_input_shaping(padd_train,T,tau0):
    t1 = padd_train.shape[0]-(3*T-1)-1
    a1 = padd_train.shape[1]-(tau0-1)
    n_train = t1 * a1 # number of training samples
    delta0 = int(np.round((tau0-1)/2))
    
    xtrain_encoder = np.empty((n_train,T ,tau0)) #shaping
    xtrain_encoder[:] = np.NaN 
    xtrain_decoder = np.empty((n_train,T ,tau0)) #shaping
    xtrain_decoder[:] = np.NaN 
    ytrain = np.empty((n_train,T ,tau0)) 
    ytrain[:] = np.NaN
    
    for t0 in range(0,t1):   # t=time, t0 = 0..39 => year 1950..1990      df_train_cbind.shape = 50,104
        for a0 in range(0,a1): # a=age,  a0=0-99 a0 => age 0..99
            pattern_idx = t0*a1 + a0
            xtrain_encoder[pattern_idx,:,:] = padd_train.iloc[t0 : (t0 + T), a0 : (a0 + tau0)].copy()  # copy years from t0 to t0+5 and ages from a0 to a0+5 into xt_train [100*t0+a0, :, :]
            xtrain_decoder[pattern_idx,:,:] = padd_train.iloc[(t0 + T) : (t0 + 2*T), a0 : (a0 + tau0)].copy()  # copy years from t0+5 to t0+10 and ages from a0 to a0+5 into xt_train [100*t0+a0, :, :]
            ytrain[pattern_idx,:,:] = padd_train.iloc[(t0 + 2*T) : (t0 + 3*T), a0 : (a0 + tau0)].copy() 
...
def preprocessed_data(raw_data_all, raw_data_train, gender, T, tau0, model = "LSTM"):
    ...
    if model == "LSTM":
        ...
        padd_train= padding(train_gender,3*T, tau0)
        xe,xd,y = transformer_input_shaping(padd_train,T,tau0)
